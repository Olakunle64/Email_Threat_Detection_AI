...
# from data_for_testing import spam_dict, non_spam_dict
...
email_pipeline = Pipeline([
    ...
    ("classifier", load_model("re_logistic_regression_model.pkl"))  # Load a pre-trained model
])

# function to test the model
def test_logistic_regression_model(email_text):
    """
...
for email_text in inbox:
    headers = extract_headers(email_text)
    print(f"Testing email '{headers['subject']}': {test_logistic_regression_model(email_text)}")

spam = fetch_and_format_emails("SPAM")
for email_text in spam:
    headers = extract_headers(email_text)
    print(f"Testing spam email '{headers['subject']}': {test_logistic_regression_model(email_text)}")